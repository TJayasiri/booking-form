...
export default function BookingFormApp() {
  ...
  // add just below: const [saving, setSaving] = useState(false);
...
const onSubmit = async () => {
  ...
  const payload = {
    ...
    ts: new Date().toISOString(),
    terms: {
      accepted: true, // or ackTnC if you prefer
      version: "2025-08-17",
      url: "https://greenleafassurance.com/policies/terms-of-service",
    },
  };
...
export default function StatusRibbon({ refId }) {
  const [job, setJob] = useState(null);          // { current_stage, due_at, created_at, ... }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!r.ok) return; // no ribbon if API isn't ready
  ...
  const content = useMemo(() => {
    ...
    const dateMap = new Map(
      history.map((h) => [h.stage, new Date(h.at)])
    );
    ...
    >
      ...
      >
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          <strong style={{ marginRight: 6 }}>Status • {refId}</strong>
          {STAGES.map(([key, label], i) => {
            const done = i <= content.idx;
            const locked = key === "ACCEPTED_INITIATED" && i === content.idx;
              ...
              >
                <span
                  style={{
                    ...
                    height: 10,
                    borderRadius: 999,
                    background: done ? (locked ? "#0ea5e9" : "#62BBC1") : "#374151",
                    border: `1px solid ${done ? (locked ? "#0ea5e9" : "#62BBC1") : "#64748b"}`,
                  }}
                />
                <div>
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-white/80 border-b border-neutral-200">
        ...
        <!-- STATUS BAR -->
<div id="glb-status" data-ref="GLB-25-445842-IIZT"></div>

<style>
  .glb-bar{background:#0b0b0c;color:#fff;padding:10px 14px;border-bottom:3px solid #62BBC1;
           display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center}
  .glb-steps{display:flex;gap:12px;flex-wrap:wrap;align-items:center}
  .glb-step{display:flex;align-items:center;gap:8px}
  .glb-dot{width:10px;height:10px;border-radius:999px;background:#374151;border:1px solid #64748b}
  .glb-step.done .glb-dot{background:#62BBC1;border-color:#62BBC1}
  .glb-step.locked .glb-dot{background:#0ea5e9;border-color:#0ea5e9}
  .glb-label{font-weight:700;font-size:12px}
  .glb-date{font-size:11px;color:#cbd5e1}
  .glb-count{font-weight:700;font-variant-numeric:tabular-nums}
</style>

<script>
(async function(){
  const root = document.getElementById('glb-status');
  if(!root) return;
  const ref = root.dataset.ref;

  // Fetch job data from your Vercel API (read-only)
  async function fetchJob(){
    const r = await fetch(`/api/job?ref=${encodeURIComponent(ref)}`);
    if(!r.ok) return null;
    return await r.json(); // { job:{current_stage,due_at,created_at,...}, history:[{stage,at},...] }
  }
  ...
  function render(job, hist){
    const cur = job?.current_stage || 'APPLICATION_SUBMITTED';
    const idx = STAGES.findIndex(s => s[0]===cur);
    const datesByStage = new Map(hist.map(h => [h.stage, new Date(h.at)]));
    ...
    root.innerHTML = `
      <div class="glb-bar">
        <div class="glb-steps">
          ${STAGES.map(([key,label],i)=>{
            const done = i <= idx;
            const locked = key==='ACCEPTED_INITIATED' && i===idx;
            const dt = datesByStage.get(key);
            const dstr = dt ? dt.toLocaleString() : '';
            return `
              <div class="glb-step ${done?'done':''} ${locked?'locked':''}" title="${label}">
                <span class="glb-dot"></span>
                <div>
                  <div class="glb-label">${label}</div>
                  ${dstr ? `<div class="glb-date">${dstr}</div>`:''}
                </div>
              </div>
            `;
          }).join('')}
        </div>
        <div id="glb-countdown" class="glb-count"></div>
      </div>
    `;

    // countdown (10-day default if no due_at)
    const target = job?.due_at ? new Date(job.due_at) : new Date(Date.now()+10*24*3600*1000);
    const el = document.getElementById('glb-countdown');
    function tick(){
  ...
  if(!data){ root.style.display='none'; return; }
  render(data.job, data.history || []);
})();
</script>
      </header>
...
function AuditMeta({ form, setForm, touched, mark }) {
  ...
  const SERVICE_CHOICES = ["ETI","Quality Inspection","Social Audit Risk Assessment","Training","VAP-Consulting","Other Standards"];
